...
async def refund_transaction(transaction_id: uuid.UUID, db: Session = Depends(get_db)):
    ...
    #TODO
    #проблема - в текущей реализации возможен возврат только одной транзакции, то есть клиент будет запрашивать возврат
    # транзакции скажем в 1000 долл, и, хотя на балансе может быть достуна эта сумма, если это рефанд транзакции в 2000 долл,
    # то, исполнение остановится на 85 строчке. Простыми словами - если было одно пополнение в 2000 долл, то вернуть тоже можно только 2000 долл
    # , даже если есть свободные средства
    try:
        refund = stripe.Refund.create(
